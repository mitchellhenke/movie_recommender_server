...
from rnn_model.neural_networks.update_manager import Adagrad
...
from rnn_model.neural_networks.helpers import evaluation
...
class RNNBase(object):
	...
	def top_k_recommendations(self, sequence, user_id=None, k=10, exclude=None):
		...
		X[0, :len(max_length_seq), :] = np.array([[item[0]] for item in max_length_seq])
	...
	def load(self, filename):
		...
		'''
		f = open(filename, 'rb')
		param = cPickle.load(f, encoding='latin1')
		f.close()
